<template>
  <section id="foot-guide">
    ...
    <section @click="goToAdd({path:'/msite', query: {geohash}})" class="guide-item">
      ...
      <span :style="$route.path.indexOf('msite') !== -1 ? 'color:#3190e8' : ''">外卖</span>
    </section>
    <section @click="" class="guide-item">
      <svg class="icon-style">
        <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.path.indexOf('search') !== -1 ? '#findActive' : '#find'"></use>
      </svg>
      <span :style="$route.path.indexOf('search') !== -1 ? 'color:#3190e8' : ''">搜索</span>
    </section>
    <section @click="" class="guide-item">
      <svg class="icon-style">
        <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.path.indexOf('order') !== -1 ? '#orderActive' : '#order'"></use>
      </svg>
      <span :style="$route.path.indexOf('order') !== -1 ? 'color:#3190e8' : ''">订单</span>
    </section>
    <section @click="goToAdd('/profile')" class="guide-item">
      <svg class="icon-style">
        <use xmlns:xlink="http://www.w3.org/1999/xlink" :xlink:href="$route.path.indexOf('profile') !== -1 ? '#profileActive' : '#profile'"></use>
      </svg>
      <span :style="$route.path.indexOf('profile') !== -1 ? 'color:#3190e8' : ''">我的</span>
    </section>
  </section>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      goToAdd(add) {
        if (!this.$route.path.indexOf(add) || !this.$route.path.indexOf(add.path)){
          console.log(add);
        }else {
